...
import { addAnime, moveFromWatchlistToMain, moveFromMainToWatchlist } from '../services/add_data';
import { editItem } from '../services/edit_data';
import { Link } from 'react-router-dom';

function HomeWatchlistAnime() {
  const { 
    isWatchlist, 
    loading, 
    error, 
  ...
  const handleAddItem = (data) => {
    ...
    if (isEdit && editingItem) {
      editItem(editingItem.id, data, 'anime', isWatchlist)
        .then(result => {
          if (result.success) {
            fetchCurrentCollection();
            setIsOpen(false);
  ...
  const handleMoveItem = async (item) => {
    try {
      if (isWatchlist) {
        const result = await moveFromWatchlistToMain(item, 'anime');
        if (result.success) {
          console.log(result.message);
          fetchCurrentCollection();
        }
      } else {
        const result = await moveFromMainToWatchlist(item, 'anime');
        if (result.success) {
          console.log(result.message);
          fetchCurrentCollection();
        }
  ...
  const handleDeleteItem = async () => {
    if (itemToDelete) {
      const result = await deleteItem(itemToDelete.id, 'anime', isWatchlist);
      if (result.success) {
        fetchCurrentCollection();
    ...
    <div className='p-8 max-w-8xl mx-auto'>
      ...
      <div className='flex justify-between items-center mb-6'>
        <h1 className='text-2xl font-bold'>
          {isWatchlist ? 'Watchlist Anime' : 'Anime'}
        </h1>
        <div className='flex gap-4'>
          <Button className='bg-blue-500 text-white' onClick={refreshAnime}>
            Refresh
          </Button>
...
export default HomeWatchlistAnime;